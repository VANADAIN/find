<template>
  <div class="text-center">
    ...
    <div v-if="errorMessage" color="error" class="error" role="alert">{{ errorMessage }}</div>
    <div v-if="successMessage" class="success" role="alert">{{ successMessage }}</div>
    ...
    <div class="d-flex justify-center" v-if="$store.state.isUserLoggedIn">
      <div class="col-6" v-if="!loggingInLoad">
        <div class="form-group">
          <v-text-field v-model="user.id" label="ID" :rules="rules"></v-text-field>
        </div>
        <div class="form-group">
...
export default {
  ...
  methods: {
    async search() {
      const field = this.search_fields.search_username;
      if (field === "") {
